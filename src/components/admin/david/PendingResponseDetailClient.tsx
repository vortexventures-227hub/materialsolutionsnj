...
export function PendingResponseDetailClient({
  ...
  function handleSave() {
    startTransition(async () => {
      ...
      if (!result) {
        showToast('Save did not complete');
        return;
      }
  ...
  function handleApprove() {
    startTransition(async () => {
      ...
      if (!result) {
        showToast('Approval did not complete');
        return;
      }
      persistLocalDraft(actionDraft);
      setStatusLabel(result.record.status);
      showToast('Approved for send queue');
    });
  }
  ...
  function handleReject() {
    startTransition(async () => {
      ...
      if (!result) {
        showToast('Rejection did not complete');
        return;
      }
      setStatusLabel(result.record.status);
  ...
  function handleEscalate() {
    startTransition(async () => {
      ...
      if (!result) {
        showToast('Escalation did not complete');
        return;
      }
      persistLocalDraft(actionDraft);
      setStatusLabel(result.record.status);
      showToast('Escalation queued for review');
    });
  }
    ...
    <div className="mx-auto flex max-w-5xl flex-col gap-5 px-4 py-6 sm:px-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#E8B800]">
              {response.meta.channel_label} draft review
            </p>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="flex flex-col gap-4">
          <section className="rounded-lg border border-white/10 bg-white/[0.03] p-4 sm:p-5">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-lg font-semibold text-white">Edit David Draft</h2>
              <div className="text-right text-xs text-slate-400">
                <p>Autosaves locally every 3 seconds</p>
          ...
          <section className="rounded-lg border border-white/10 bg-white/[0.03] p-4 sm:p-5">
            <h2 className="text-lg font-semibold text-white">Runtime Details</h2>
            <dl className="mt-4 grid gap-3 text-sm">
              ...
              <div className="rounded-lg border border-white/10 bg-black/20 p-3">
                ...
                <dd className="mt-2 whitespace-pre-wrap text-slate-100">
                  {response.reasoning ?? 'No runtime notes attached.'}
                </dd>
              </div>
            </dl>
          </section>
          ...
          <section className="rounded-lg border border-white/10 bg-white/[0.03] p-4 sm:p-5">
            <h2 className="text-lg font-semibold text-white">Decision Controls</h2>
